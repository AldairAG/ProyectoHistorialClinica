    ...
    return (
        <main className=" bg-gray-100 border border-gray-300 rounded-sm p-4 flex flex-col gap-4">
            ...
            {recetasList.map((item, index) => (
                <div className="bg-gray-50 rounded-md border border-gray-300 p-4 flex-col flex gap-2" key={index}>
                    <label className="text-lg text-gray-600 font-semibold">Receta del {item.fechaCreacion}</label>

                    {item.tratamientos.map((item, index) => (
